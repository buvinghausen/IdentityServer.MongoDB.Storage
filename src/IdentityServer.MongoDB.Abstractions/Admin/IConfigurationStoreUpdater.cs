...
namespace IdentityServer.MongoDB.Abstractions.Admin
{
	public interface IConfigurationStoreUpdater<T>
	{
		...
		IConfigurationStoreUpdater<T> AddToSetEach<TItem>(Expression<Func<T, IEnumerable<TItem>>> field,
		...
		IConfigurationStoreUpdater<T> PullAll<TItem>(Expression<Func<T, IEnumerable<TItem>>> field,
			IEnumerable<TItem> values);

		IConfigurationStoreUpdater<T> PullFilter<TItem>(Expression<Func<T, IEnumerable<TItem>>> field,
