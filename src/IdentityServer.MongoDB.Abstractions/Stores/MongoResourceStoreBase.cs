...
internal abstract class MongoResourceStoreBase<TResource, TIdentityResource, TApiResource, TApiScope, TResources> : MongoStoreBase<TResource> where TIdentityResource : TResource where TApiResource : TResource where TApiScope : TResource
{
	protected MongoResourceStoreBase(ConfigurationStoreOptionsBase options) :
		base(options.Database, options.ResourceCollectionName)
	{
	}

	public Task<IEnumerable<TIdentityResource>> FindIdentityResourcesByScopeNameAsync(IEnumerable<string> scopeNames) =>
		FindIdentityResourcesByScopeNameAsync(scopeNames, CancellationToken.None);

	public Task<IEnumerable<TIdentityResource>> FindIdentityResourcesByScopeNameAsync(IEnumerable<string> scopeNames, CancellationToken cancellationToken) =>
	...
	public Task<IEnumerable<TApiScope>> FindApiScopesByNameAsync(IEnumerable<string> scopeNames, CancellationToken cancellationToken) =>
	...
	public Task<IEnumerable<TApiResource>> FindApiResourcesByNameAsync(IEnumerable<string> apiResourceNames, CancellationToken cancellationToken) =>
	...
	public abstract Task<IEnumerable<TApiResource>> FindApiResourcesByScopeNameAsync(IEnumerable<string> scopeNames, CancellationToken cancellationToken);

	protected abstract Task<IEnumerable<T>> FindResourcesByName<T>(IEnumerable<string> names, CancellationToken cancellationToken = default) where T : TResource;
