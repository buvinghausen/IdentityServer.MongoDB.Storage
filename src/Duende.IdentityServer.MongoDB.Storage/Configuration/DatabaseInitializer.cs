...
public sealed class DatabaseInitializer : IDatabaseInitializer
{
	...
	public DatabaseInitializer(ConfigurationStoreOptions configurationStoreOptions) :
		this(configurationStoreOptions, default)
	{
	}
	...
	public Task InitializeConfigurationStoreAsync(CancellationToken cancellationToken = default) =>
		DatabaseInitializerBase.InitializeConfigurationStoreAsync<Client, Resource>(c => c.AllowedCorsOrigins, r => r.Name, new[] { _configurationStoreOptions?.IdentityProviderCollectionName }, _configurationStoreOptions, cancellationToken);

	public Task InitializeOperationalStoreAsync(CancellationToken cancellationToken = default) =>
		DatabaseInitializerBase.InitializeOperationalStoreAsync<DeviceFlowCode, PersistedGrant>(dc => dc.DeviceCode, dc => dc.Expiration, pg => pg.SubjectId, pg => pg.ClientId, pg => pg.SessionId, pg => pg.Type, pg => pg.Expiration, pg => pg.ConsumedTime, new []{ _operationalStoreOptions?.SigningKeyCollectionName }, _operationalStoreOptions, cancellationToken);
}
