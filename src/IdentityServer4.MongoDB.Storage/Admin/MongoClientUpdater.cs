...
public sealed class MongoClientUpdater : MongoStoreUpdaterBase<Client>
{
	...
	public MongoClientUpdater(ConfigurationStoreOptions options) :
		this(options.Database, options.ClientCollectionName)
	{
	}

	public MongoClientUpdater(IMongoDatabase database, string collectionName) :
		base(database.GetCollection<Client>(collectionName))
	{
	}
