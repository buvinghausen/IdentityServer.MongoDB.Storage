...
internal sealed class DatabaseInitializer : IHostedService
{
	...
	public async Task StartAsync(CancellationToken cancellationToken)
	{
		...
		await Task.WhenAll(
			...
			_identityProviderUpdater.InsertManyAsync(new IdentityProvider[]
			{
				new("oidc")
				{
					Scheme = "demoidsrv",
		...
		/*
		// Step 4 - Show how updates work in the event you need to change the whole document or just part(s)
		await _clientUpdater
			.Set(c => c.Enabled, false) // <-- this will disable the client
			.PopLast(c => c.ClientSecrets) // <-- this will remove the password
			.UpdateOneAsync(c => c.ClientId == "not-postman", cancellationToken);

		await _clientUpdater
			.AddToSet(c => c.ClientSecrets,
				new Secret("new-postman-secret".ToSha256(), "This secret was added later"))
			.Inc(c => c.AccessTokenLifetime, 10) // <- add 10 seconds to the token lifetime
			.UpdateOneAsync(c => c.ClientId == "postman", cancellationToken);

		await _clientUpdater
			.PullFilter(c => c.ClientSecrets,
				s => s.Description == null) // <-- this will remove items from an array based on a filter predicate
			.UpdateOneAsync(c => c.ClientId == "postman", cancellationToken);
		*/
	}
